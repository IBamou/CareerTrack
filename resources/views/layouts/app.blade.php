...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="font-sans antialiased bg-gray-50 dark:bg-gray-950" x-data="{ sidebarOpen: false, sidebarCollapsed: document.documentElement.style.getPropertyValue('--sidebar-w').trim() === '3.5rem', archivesOpen: @json(request()->routeIs('*.archives')) }" x-init="$watch('sidebarCollapsed', val => { localStorage.setItem('sidebarCollapsed', val); document.documentElement.style.setProperty('--sidebar-w', val ? '3.5rem' : '14rem'); })">
